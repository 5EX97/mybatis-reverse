package com.example.generate_code.ontroller;
...
public class DbController {
    @PostMapping("/connect")
    public RespBean connect(@RequestBody Db db){//RequestBody表明前端传的是一个json字符串，会自动解析成Db对象
        Connection con = DBUtils.initDb(db);
        if(con!=null){
            return RespBean.ok("数据库连接成功");
        }
        return RespBean.error("数据路连接失败");
    }
    ...
    public RespBean config(@RequestBody Map<String,String> map){
        ...
        try {
            ...
            DatabaseMetaData metaData = connection.getMetaData();//获取数据库的元信息
            ResultSet tables = metaData.getTables(connection.getCatalog(), null, null, null);//获取数据库中的表信息
            List<TableClass> tableClassList = new ArrayList<>();
            while(tables.next()){
                TableClass tableClass = new TableClass();
                tableClass.setPackageName(packageName);
                String table_name = tables.getString("TABLE_NAME");//获取表的名字
